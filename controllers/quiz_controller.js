...
.then(function(attachment){
req.flash('succes', 'Imagen nueva guardada con éxito.');
if (old_public_id){
...
var keyw = (req.query.search)? req.query.search.split(" ") : "%";
var aux = "%";
for (var i = 0; i < keyw.length; i++){
aux += keyw[i] + "%";
}
models.Quiz.findAll(
{where: {question: {$like: aux}},
include: [models.Attachment]}).then(function(quizzes) {
...
models.Quiz.findById(req.params.quizId) // Busca la pregunta
.then(function(quiz) {
if (quiz) {
...
.then(function(quiz){
req.flash('success', 'Pregunta y Respuestas guardadas con éxito.');
if (!req.file){
...
quiz.answer = req.body.quiz.answer;
//guarda en DB los campos pregunta y respuesta de quiz
quiz.save({fields: ["question", "answer"]})
.then(function(quiz) {
req.flash('success', 'Quiz editado con éxito.');
// Sin imagen: elimintar attachment e imagen viejos
if (!req.file){
...
//Borra la imagen de Cloudinary (Ignoro resultado)
